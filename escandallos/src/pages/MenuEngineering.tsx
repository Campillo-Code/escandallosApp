...
export default function MenuEngineering() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const result = await invoke<MenuItem[]>("get_menu_engineering", { fechaDesde, fechaHasta });
      setData(result);
  ...
  const chartData = data.map(d => ({    x: d.unidades_vendidas,
  ...
  return (
    <div className="p-6">
      ...
      {loading ? <div className="text-center text-gray-500 py-12">Cargando datos...</div> :
        data.length === 0 ? <div className="text-center text-gray-500 py-12">No hay ventas en este periodo. Importa datos desde el módulo de Ventas.</div> : (
          <>
            {/* BCG Matrix Chart */}
            <div className="bg-white rounded-lg shadow p-6 mb-6">
              ...
              <ResponsiveContainer width="100%" height={400}>
                <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                  ...
                  <XAxis type="number" dataKey="x" name="Unidades" label={{ value: "Unidades vendidas →", position: "bottom", offset: 0 }} stroke="#6b7280" />
                  <YAxis type="number" dataKey="y" name="Margen %" label={{ value: "Margen % →", angle: -90, position: "insideLeft" }} stroke="#6b7280" />
