    ...
    <div className="p-6">
      ...
      <>
        <div className="p-4 pb-2">
          <SearchBar value={searchTerm} onChange={setSearchTerm} placeholder="Buscar ficha receta..." />
        </div>
        <div className="bg-white rounded-lg shadow overflow-hidden">
            ...
            fichas.length === 0 ? <div className="p-6 text-center text-gray-500">No hay fichas de receta</div> : (
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200">
                  {fichas.filter(f => !searchTerm || f.receta_nombre?.toLowerCase().includes(searchTerm.toLowerCase())).map(f => (
